...
class TreeReader {
 ...
 public:
  TreeReader(int tg_traj_evt, int tg_traj_traj, int tg_traj_mod_on, 
	     int tg_traj_clu, int tg_traj_trk, int tg_traj_mod,
 ...
 private:
  ...
  void read_(TFile &f) {
    ...
    b_t_clu     ->SetAddress(&traj_.clu.pix);
    ...
    b_c_clu_pix ->SetAddress(&clu_.pix);
    ...
    // Paired branches
#if SPLIT > 0
    TBranch *traj_traj_occup           = trajTree_->GetBranch("traj_occup");
    ...
    // Paired branches
#if SPLIT > 0
    traj_traj_occup            -> SetAddress(&traj_.nclu_mod);
    traj_traj_alphabeta        -> SetAddress(&traj_.alpha);
    traj_dxy_cl                -> SetAddress(&traj_.dx_cl);
    traj_dxy_hit               -> SetAddress(&traj_.dx_hit);
#endif
    // Split-mode branches
    ...
    traj_clust_adc            -> SetAddress(&traj_.clu.adc);
#if SPLIT > 1
...
#if SPLIT > 0
    // trajTree_->SetBranchStatus("traj_occup",0);
 ...
 public:
  ...
  Int_t traj_read(long long int i) { return trajTree_->GetEntry(i); }
#ifdef COMPLETE
